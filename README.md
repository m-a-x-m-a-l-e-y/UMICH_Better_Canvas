...
A Chrome extension that enhances the University of Michigan's Canvas dashboard (`umich.instructure.com`) with campus event info, sports schedules, and a cleaner UI.
...
- Host permissions for `maizepages.umich.edu` and the sports data AWS Lambda endpoints are used to fetch event data.
